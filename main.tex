...
For implementation at the PS, and for use in extraction to East Area, a basic beam is set-up and configured for the East area. A beam momentum of \qty[per-mode=symbol]{5.392}{\giga\electronvolt\per~c} was chosen for this study, as it is equivalent to the \qty[per-mode=symbol]{750}{\mega\electronvolt\per~c} Lead (\ch{Pb^{+54}}) ions used for the CHIMERA experiment, with whom studies were carried out alongside, and it equates to a low enough energy as to not damage components during testing. This proton equivalent momentum is calculated as
\begin{equation}
