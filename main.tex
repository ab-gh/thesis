...
% [TODO) talk about initial particle distribution generation, matching measurements taken from the machine
...
% [TODO) PRESENT simulation time results, and how much faster GPU acceleration is



\section{Frequency Response}
% [TODO) talk about what exactly was measured - spill quality, duty cycle, frequency response
...
The time-series spill signal was then analysed using a discrete Fourier transform (DFT) to obtain the frequency response of the system. Using a peak-finding algorithm to allow for slight variation in spill measurement frequency---over the measurements used, the \verb|BXSCINT| sample rate varied between \qty{18.6}{\kilo\hertz} and \qty{18.95}{\kilo\hertz} (\autoref{fig:sample_rate_variation}). As this variation was evenly distributed around the mean value of \qty{18.78}{\kilo\hertz}, the DFT was largely unaffected.

\begin{figure}
  \centering
  \includegraphics*[width=0.8\linewidth]{sample_rate_variation.png}
  \caption{Variation in sample rate of the \texttt{BXSCINT} device over the course of the measurement.}\label{fig:sample_rate_variation}
\end{figure}

Once the DFT was computed, the interval rate for each measurement was selected, normalised by the DFT DC component (the \qty{0}{\hertz} term, equivalent to the average of all samples), and the magnitude of the resulting complex number was plotted against the interval frequency. This analysis was performed for both the real machine and the simulations, with both the RF systems on (\autoref{fig:rf_on}) and off. 
