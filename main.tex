...
\begin{figure}[h]
  ...
  \draw   (104.63,81.43) -- (233.66,81.43) -- (233.66,168.29) -- (104.63,168.29) -- cycle ;
  %Shape: Rectangle [id:dp004915718994831897] 
  \draw   (104.63,66.95) -- (233.66,66.95) -- (233.66,81.43) -- (104.63,81.43) -- cycle ;
  %Shape: Rectangle [id:dp9834216786415646] 
  \draw   (104.63,168.29) -- (233.66,168.29) -- (233.66,182.77) -- (104.63,182.77) -- cycle ;
  %Straight Lines [id:da9215546463069324] 
  \draw    (123.07,95.91) -- (123.07,150.81) ;
  \draw [shift={(123.07,153.81)}, rotate = 270] [fill={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.08]  [draw opacity=0] (8.93,-4.29) -- (0,0) -- (8.93,4.29) -- cycle    ;
  %Straight Lines [id:da4829295704205643] 
  \draw    (215.23,95.91) -- (215.23,150.81) ;
  \draw [shift={(215.23,153.81)}, rotate = 270] [fill={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.08]  [draw opacity=0] (8.93,-4.29) -- (0,0) -- (8.93,4.29) -- cycle    ;
  %Straight Lines [id:da5246257542758568] 
  \draw    (169.15,95.91) -- (169.15,150.81) ;
  \draw [shift={(169.15,153.81)}, rotate = 270] [fill={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.08]  [draw opacity=0] (8.93,-4.29) -- (0,0) -- (8.93,4.29) -- cycle    ;
  %Flowchart: Summing Junction [id:dp9967505452618075] 
  \draw   (161.85,124.86) .. controls (161.85,120.83) and (165.12,117.56) .. (169.15,117.56) .. controls (173.18,117.56) and (176.45,120.83) .. (176.45,124.86) .. controls (176.45,128.89) and (173.18,132.16) .. (169.15,132.16) .. controls (165.12,132.16) and (161.85,128.89) .. (161.85,124.86) -- cycle ; \draw   (163.99,119.7) -- (174.31,130.02) ; \draw   (174.31,119.7) -- (163.99,130.02) ;
  %Straight Lines [id:da705900304054244] 
  \draw    (169.15,124.86) -- (198.2,124.86) ;
  \draw [shift={(200.2,124.86)}, rotate = 180] [color={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.75]    (10.93,-3.29) .. controls (6.95,-1.4) and (3.31,-0.3) .. (0,0) .. controls (3.31,0.3) and (6.95,1.4) .. (10.93,3.29)   ;
  
  % Text Node
  \draw (150.15,45.7) node [anchor=north west][inner sep=0.75pt]   [align=left] {\begin{minipage}[lt]{27.66pt}\setlength\topsep{0pt}
  \begin{center}
  North
  \end{center}
  ...
  \end{minipage}};
  % Text Node
  \draw (148.65,188.02) node [anchor=north west][inner sep=0.75pt]   [align=left] {\begin{minipage}[lt]{29.38pt}\setlength\topsep{0pt}
  \begin{center}
  South
  \end{center}
  ...
  \end{minipage}};
  % Text Node
  \draw (108.27,101.07) node [anchor=north west][inner sep=0.75pt]   [align=left] {\begin{minipage}[lt]{9.78pt}\setlength\topsep{0pt}
  \begin{center}
  $\displaystyle \vec{B}$
  \end{center}
  ...
  \end{minipage}};
  % Text Node
  \draw (191.27,98.07) node [anchor=north west][inner sep=0.75pt]   [align=left] {\begin{minipage}[lt]{9.14pt}\setlength\topsep{0pt}
  \begin{center}
  $\displaystyle \vec{F}$
  \end{center}
...
The mechanism behind the spill dury factor improvement with bunching can be understood by examining how the stable phase space behaves with respect to synchrotron oscillation. As covered in~\autoref{sec:long_phase_space}, particles moving within longitudinal RF buckets will undergo longitudinal oscillations around the synchronous particle, known as synchrotron oscillations\footnote{As opposed to the transverse betatron oscillations.}. This can be calculated as
\begin{equation}
  Q_s = \sqrt{\frac{hqV_{rf}|\eta|\cos\phi_s}{2\pi\beta_0^2E_{tot}}}
\end{equation} where $h$ is the RF harmonic, $V_{rf}$ the RF voltage, $q$ the particle charge, $\eta$ the slip factor (\autoref{eq:slip_factor}), $\phi_s$ the synchronous phase, and $E_{tot}$ the total energy. It can also be found by simulation; for the configuration outlined in preceeding sections, the synchrotron tune was found to be $Q_s = 0.0052$. 
In a Steinbach diagram (\autoref{sec:steinbach}), this has the effect of shifting the particle horizontally. This has two consequences: the particles remain at the edge of the unstable region for a shorter amount of time, as the synchrotron motion changes the particle's tune enough to move it into resonance, reducing the effect of the excitation structures. Additionally, it can reduce the effect of magnet ripples changing the shape of the unstable region.

\begin{figure}
  \centering
  \includegraphics*[width=0.7\linewidth]{synch-motion.png}
  \caption{Steinbach diagrams showing the effect of synchrotron motion in a bunched beam (bottom) compared to a coasting beam (top). For a small sextupole ripple, reducing the unstable region width (stopband), a coasting particle approaching resonance may become \textit{recaptured} into the stable region before amplitude growth and extraction can occour. A bunched particle, however, would have a higher probability of remaining inside the unstable region, as the synchrotron motion changes the tune towards resonance.}\label{fig:synch-steinbach}
\end{figure}

To see this, we will consider the two cases of a particle approaching resonance in a coasting beam and in a bunched beam, visualised with a Steinbach diagram. Illustrated in~\autoref{fig:synch-steinbach}, particles undergoing slow synchrotron motion in a bunched beam would have a higher chance of remaining inside the unstable region than a coasting beam during changes in stopband width (e.g. due to sextupole ripple)~\cite{Sorge_2018}. This effect diminishes at higher synchrotron tunes, however. As the particle's syncrotron period decreases, has a higher probability of moving back out of resonance too fast to be captured, and this ripple mitigation effect reduces. The bucket hight (synchrotron oscillation amplitude) is proportional to $\sqrt{h}$ (harmonic number); for PS East extraction, at the \nth{8} harmonic, this ripple mitigation effect is still present. 

This synchrotron motion also effects the extraction transit time. The distance of a particle from the separatrix while in the unstable region grows with each synchrotron period. This reduces the transit time. Therefore, there exists an optimum synchrotron tune, where particles are more likely to remain within the unstable region, but the transit time remains long enough to smoothen the spill ``pulses'' between successive excitations (\autoref{fig:sdf_example}).
